...
import copy
import numpy as np
from PPOMixDAGLoader import OffloadingDAG
...
class OffloadingEnvironment(object):
    ...
    def load_data(self, params):
        ...
        if params['load_type'] == 'gv':
            for graph_file_path in params['graph_file_path']:
                ...
                list_task_sequence.append(np.array(task_sequence_list_in_file))
                list_dependency_matrix.append(np.array(dependency_list_in_file))

        elif params['load_type'] == 'npy':
            for data_path in params['numpy_file_path']:
                self.logger.info('loading data from npy file path: {}'.format(data_path))
                list_task_graph.append(None)
                list_task_sequence.append(np.load(data_path + '_task_sequence' + '.npy'))
                list_dependency_matrix.append(np.load(data_path + '_dependency_matrix' + '.npy'))
    ...
    def get_train_setting(self, **train_set):
        # check instance_num & batch_size
        if train_set['pref_flag'] == 'step':
            min_instance = int(1 / train_set['pref_step'] + 1) * train_set['batch_size']
        elif train_set['pref_flag'] == 'random':
            min_instance = train_set['pref_per_epoch'] * train_set['batch_size']
        else:
            raise AssertionError('Parameter \'pref_flag\' must be \'step\' or \'random\'!!!')

        for task_sequence in self.dataset['task_sequence']:
            self.logger.info('--------------------------------------------------------------------------')
            dataset_num = task_sequence.shape[0]
            instance_num = max(min_instance, dataset_num)
            self.instance_num.append(copy.deepcopy(instance_num))

            self.logger.info('Task num {:d} train data setting is:'.format(task_sequence.shape[1]))
            self.logger.info('Train epoch instance num: {:d}'.format(instance_num))
            ...
            self.logger.info('Each epoch will train with {:d} sets of preferences'.format(int(
                np.ceil(instance_num/train_set['batch_size']))))

    def init_epoch(self, train_set=None):
        self.epoch_idx = []
        self.epoch_pref = []
        self.epoch_class_count = 0

        for task_sequence, instance_num in zip(self.dataset['task_sequence'], self.instance_num):

            # generate data index
            if instance_num > task_sequence.shape[0]:
                perm = np.repeat(np.arange(task_sequence.shape[0]),
                                 np.ceil(instance_num / task_sequence.shape[0]), axis=0)
                np.random.shuffle(perm)
                self.epoch_idx.append(copy.deepcopy(perm[:instance_num]))
            else:
                perm = np.arange(instance_num)
                np.random.shuffle(perm)
                self.epoch_idx.append(copy.deepcopy(perm))
            ...
            if train_set:
                ...
                if train_set['pref_flag'] == 'step':
                    ...
                    ori_pref = np.stack([pref0, pref1], axis=1)
                    shuffle_pref = ori_pref.repeat(np.ceil(pref_num / pref0.shape[0]), axis=0)
                    np.random.shuffle(shuffle_pref)
                    pref = np.vstack([ori_pref, shuffle_pref])[:pref_num]
                elif train_set['pref_flag'] == 'random':
                    pref0 = np.random.rand(int(pref_num))
                    pref1 = np.ones(int(pref_num)) - pref0
                    pref = np.stack([pref0, pref1], axis=1)
                else:
                    raise AssertionError('Parameter \'pref_flag\' in \'runner_params\' must be \'step\' or \'random\' !!!')
                np.random.shuffle(pref)
                self.epoch_pref.append(copy.deepcopy(pref))

    def load_batch(self, batch_size):
        if len(self.epoch_idx[self.epoch_class_count]) == 0:
            self.epoch_class_count += 1
        task_choose = self.epoch_class_count
        idx = self.epoch_idx[task_choose][:batch_size]
        self.epoch_idx[task_choose] = self.epoch_idx[task_choose][batch_size:]
        self.pref = self.epoch_pref[task_choose][0]
        self.epoch_pref[task_choose] = self.epoch_pref[task_choose][1:]

        # [batch, task_num, 17]
        for key in self.baseline:
            self.batch_baseline[key] = copy.deepcopy(self.baseline[key][task_choose][idx])
        self.problems = copy.deepcopy(self.dataset['task_sequence'][task_choose][idx])
        self.dependencies = copy.deepcopy(self.dataset['dependency_matrix'][task_choose][idx])
    ...
    def get_local_baseline(self):
        local_le = []
        for task_sequence in self.dataset['task_sequence']:
            latency_local = task_sequence[:, :, 1]
            energy_local = self.mec_resource.energy_cost(latency_local, 'local')
            local_le.append(np.stack([latency_local, energy_local], axis=-1))
        self.baseline.update(local_le=local_le)

    def get_remote_baseline(self):
        remote_le = []
        for task_sequence, dependency_matrix in zip(self.dataset['task_sequence'], self.dataset['dependency_matrix']):
            action = np.ones(np.shape(task_sequence)[1], dtype=np.int32)
            for problem, dependency in zip(task_sequence, dependency_matrix):
                self.get_pre_reward_for_one_problem(action, problem, dependency)

            remote_le.append(np.stack([self.latency, self.energy], axis=-1))
            self.latency = []
            self.energy = []
